...
var ColumnChartComponent = forwardRef(function (props, ref) {
  var _ref = props,
      labels = _ref.labels,
  ...
  useEffect(function () {
    deprecationNotice('ColumnChart', "This component is deprecated and will be removed with v0.10.0. Please use '@ui5/webcomponents-react-charts/lib/next/ColumnChart' instead.");
  }, []);
  var theme = getTheme();
  ...
  var columnChartDefaultConfig = useMemo(function () {
    return {
      scales: {
      ...
      plugins: {
        datalabels: {
          display: function display(context) {
            ...
            var dataMeta = datasetMeta.data[context.dataIndex];
            var height = dataMeta._view.base - dataMeta._view.y; // offset

            if (height < getTextHeight() + 6) {
              return false;
            }

            var formattedValue = valueAxisFormatter(context.dataset.data[context.dataIndex], context.dataset, context);
            var textWidth = getTextWidth(formattedValue);
          ...
          color: function color(context) {
            var datasetMeta = context.chart.getDatasetMeta(context.datasetIndex);
            var dataMeta = datasetMeta.data[context.dataIndex];
            return bestContrast(dataMeta._view.backgroundColor, [getCssVariableValue('--sapTextColor', '#32363a'), getCssVariableValue('--sapContent_ContrastTextColor', '#fff')]);
          }
        }
      }
    };
  }, [categoryAxisFormatter, valueAxisFormatter]);
  var mergedOptions = useMergedConfig(columnChartDefaultConfig, options);
...
var ColumnChart = withChartContainer(ColumnChartComponent);
ColumnChart.defaultProps = _objectSpread({}, ChartBaseDefaultProps);
